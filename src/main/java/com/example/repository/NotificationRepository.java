...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Query(
        value="SELECT * FROM (SELECT n.*, ROW_NUMBER() OVER(ORDER BY n.no DESC) rnum FROM NOTIFICATION n WHERE n.receiver = :id ) n WHERE rnum BETWEEN 1 AND 10 ORDER BY n.rnum ASC ", 
